...
app.use(
  cors(["http://localhost:3000", "https://social-app-mongo-search.vercel.app/"])
);
...
app.get("/api/v1/stories", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .send({ message: "failed to get stories, please try later" });
  }
});
...
app.delete("/api/v1/story/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .send({ message: "failed to delete story, please try later" });
  }
});
